...
CasperTest.begin('About screen is correct', 9, function suite(test) {
    ...
    casper.waitForSelector('.gh-contributors li', function testContributors() {
        var firstContribImageSrc = casper.getElementAttribute('.gh-contributors li:nth-child(1) a img', 'src');

        // Check first contributor image tag is on the page
        test.assertExist('.gh-contributors li:nth-child(1) img', 'First contributor image is in place');
        ...
        test.assertDoesntExist('.gh-contributors li:nth-child(1) a img[alt=""]', 'First contributor image alt is not empty');

        // Check first contributor links to GitHub
        test.assertExists('.gh-contributors li:nth-child(1) a[href*="github.com"]', 'First contributor link to GitHub');

        // Check first contributor links to GitHub
        test.assertDoesntExist('.gh-contributors li:nth-child(1) a[title=""]', 'First contributor title is not empty');
    });
});
